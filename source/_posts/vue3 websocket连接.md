---
title: "vue3 websocket连接"
catalog: true
date: 2025-05-14 13:47:29
...
const initWebSocket = () => {
	...
	let wssUri = import.meta.env.MODE === 'production' ? `wss://${host}${props.uri}?access_token=${token.value}&TENANT-ID=${tenant.value}` : wsUri;
	// 建立连接
	state.webSocket = new WebSocket(wssUri);
...
const Routers = useTagsViewRoutes()
...
const onMessage = (msgEvent: any) => {
	console.log(msgEvent, 'websocket--msgEvent')
	//收到服务器信息，心跳重置并发送
	startHeartbeat();
	const text = JSON.parse(msgEvent.data);
	console.log(text.contents, '---msgEvent---')
};
